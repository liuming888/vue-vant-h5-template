 ...
 */
function notifyMe(registration) {
    var isMS = OSnow() == 'mac' && isBrowser() == 'Safari';
    console.log('可能不准isBrowser(): ', isBrowser());
    console.log(' 可能不准OSnow(): ', OSnow());
    console.warn('可能不准isMS: ', isMS);
    var title = 'PWA即学即用';
    var options = {
    ...
    if (!('Notification' in window)) {
        ...
        try {
            if (!isMS) {
                ...
                //  目前移动端浏览器普遍还不支持上面这种。但是在Mac OS上的safari里面是支持该特性的，不过其调用方式与上文代码有些不太一样
                notification = new Notification('Hi there!', options);
            }
        } catch (error) {
            console.log('new Notification 错误', error);
            registration.showNotification(title, options);
        }
    ...
    registerServiceWorker('./swdemo.js')
        ...
        .then(function(result) {
            ...
            document.querySelector('#pwaT').addEventListener('click', function() {
                console.warn('点击了');
                //  Notification API调用（主要pc端）
                notifyMe(registration);
            });
